...
def RNN_LSTM(x, Weights, biases):
    ...
    # cell 中的 dropout
    def attn_cell():
        lstm_cell = tf.contrib.rnn.GRUCell(n_hiddens)
...
with tf.Session() as sess:
    sess.run(init)
    train_writer = tf.summary.FileWriter("./logs/gru_arousal_BN/train",sess.graph)
    test_writer = tf.summary.FileWriter("./logs/gru_arousal_BN/test",sess.graph)
